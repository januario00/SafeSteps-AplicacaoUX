...
export function MapView({ latitude, longitude, accuracy, occurrences = [], showPoliceStations = false }: MapViewProps) {
  ...
  return (
    <div className="relative w-full h-64 rounded-lg overflow-hidden border-2 border-gray-200" style={{ background: 'linear-gradient(to bottom right, #E8EDF2, #C5D0DC, #E8EDF2)' }}>
      {/* Grid do mapa */}
      ...
      {occurrences.length > 0 && occurrencePositions.map((pos, index) => (
        ...
        >
          <AlertCircle className="w-6 h-6 drop-shadow-lg cursor-pointer hover:scale-110 transition-transform" style={{ color: '#798DA7' }} fill="currentColor" />
      ...
      {policeStationPositions.map((station, index) => (
        ...
        >
          <div className="rounded-full p-2 shadow-lg cursor-pointer hover:scale-110 transition-transform" style={{ backgroundColor: '#5D6F85' }}>
      ...
      >
        <MapPin className="w-10 h-10 drop-shadow-lg animate-bounce" style={{ color: '#798DA7' }} fill="currentColor" />
      </div>
        ...
        <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm px-2 py-1 rounded text-xs space-y-1">
          {occurrences.length > 0 && (
            <div className="flex items-center gap-1">
              <AlertCircle className="w-3 h-3" style={{ color: '#798DA7' }} fill="currentColor" />
              <span>{occurrences.length} Ocorrência{occurrences.length !== 1 ? 's' : ''}</span>
            </div>
          )}
          {showPoliceStations && (
            <div className="flex items-center gap-1">
              <Phone className="w-3 h-3" style={{ color: '#5D6F85' }} />
