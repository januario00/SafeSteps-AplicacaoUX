...
export function VoiceMode({ onClose, location, onRegisterOccurrence, showPoliceStations }: VoiceModeProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'linear-gradient(to bottom right, #798DA7, #5D6F85, #465363)' }}>
      {/* Header */}
        ...
        {location && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
            <div className="relative">
              <div className="w-4 h-4 rounded-full border-4 border-white shadow-lg animate-pulse" style={{ background: '#798DA7' }}></div>
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 rounded-full animate-ping" style={{ background: 'rgba(121, 141, 167, 0.3)' }}></div>
            </div>
          </div>
        )}
      ...
      <div className="p-6 space-y-4">
        ...
        <Card className="p-4 bg-white/95 backdrop-blur">
          <div className="flex items-start gap-3">
            <Bot className="w-6 h-6 flex-shrink-0 mt-1" style={{ color: '#798DA7' }} />
            <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Button
            ...
            style={
              isListening
                ? { background: '#5D6F85', animation: 'pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite' }
                : { background: 'linear-gradient(to right, #798DA7, #5D6F85)' }
            }
          ...
          <Button
            size="lg"
            className="h-16"
            style={{ background: 'linear-gradient(to right, #465363, #5D6F85)' }}
            onClick={() => {
          ...
          <div className="bg-white/90 backdrop-blur rounded-lg p-3 space-y-2">
            ...
            {policeStations.map((station, index) => (
              <div key={index} className="text-xs text-gray-700 flex items-center justify-between">
                ...
                <span style={{ color: '#798DA7' }}>{station.distance}</span>
              </div>
            ))}
          </div>
        )}
